...
class UsersController < ApplicationController
  ...
  before_action :messaging, only: ['inbox']
  ...
  def inbox
    ...
    return if @chats.empty? || @chats.nil?
  ...
  def send_friend_request
    ...
    @request = current_user.sent_friend_requests.create(receiver:, status: 'pending')
  ...
  def remove_friend
    ...
    friendship = Friendship.find_by(user_id: current_user.id,
                                    friend_id: friend.id) || Friendship.find_by(
