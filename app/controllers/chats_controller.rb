...
class ChatsController < ApplicationController
  ...
  def show
    ...
    if Chat.all.empty?
      ...
      current_user.notifications.includes(:event).where(noticed_events: { record_type: 'Message',
                                                                          params: { chat: @chat } }).unread.mark_as_read
  ...
  def create
    ...
    Rails.logger.debug "User 2 id: #{@user_2.id}"
    @chat = @user.chats.new(user_id: @user.id, user2_id: @user_2.id)
    Rails.logger.debug "User2 id again: #{@chat.user2_id}"
    @chat.save
    @message = @chat.messages.build(user_id: @user.id, chat_id: @chat.id,
                                    content: chat_params[:message_attributes][:content])
    Rails.logger.debug "Message User id: #{@message.user_id}"
    if @message.save
