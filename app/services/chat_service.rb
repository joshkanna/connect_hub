...
class ChatService
  ...
  def call
    return unless messages.last.user.username != 'chatbot'
    ...
    messages.each do |message|
      chat_messages << if message.user == User.find_by(username: 'chatbot')
    ...
    @message = Message.create(chat_id: messages.first.chat.id, user_id: User.find_by(username: 'chatbot').id,
